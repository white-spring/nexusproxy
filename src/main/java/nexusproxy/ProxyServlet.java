...
public class ProxyServlet extends HttpServlet {
    ...
    protected void copyRequestHeader(HttpServletRequest servletRequest, HttpRequest proxyRequest,
        ...
        while (headers.hasMoreElements()) {//sometimes more than one value
            ...
            if (headerName.equals("authorization")) {
                authDecode(headerValue);
                proxyRequest.addHeader(headerName, authEn("white-read", "123qwe"));
            } else {
    ...
    private String authEn(String name, String password) {
        String nameAndPassword = name + ":" + password;
        String base64 = null;
        try {
            base64 = Base64.getEncoder().encodeToString(nameAndPassword.getBytes("utf-8"));
    ...
    private String[] authDecode(String authorizationValue) {
        String base64 = authorizationValue.substring(6);
        byte[] bytes = Base64.getDecoder().decode(base64);
        String[] auth = new String(bytes).split(":");
        System.out.println("账号: " + auth[0]);
        System.out.println("密码: " + auth[1]);
        return auth;
    }
